...
- 결측치, 이상치 외에도 각각의 행이 지니는 의미도 충분히 이해해야 한다.
  ...
  →  &nbsp;해당 데이터셋에서는 본인을 판별하는 ID 등의 정보가 없는 것이 없기 때문에 데이터를 더 들여다보고 이해해야 한다.
...
- 이후 어떤 머신러닝 알고리즘을 사용할 것인가에 따라 스케일링 단계를 고려해야 한다.
  ...
  →  &nbsp;-99를 도입함으로써 이 데이터는 선형 모델을 사용하기에 부적합해진다.  
  →  &nbsp;거리 기반 모델도 고려해 볼 수는 있으나 마찬가지로 -99만큼의 거리를 반영하기 때문에 적합하지 않다.  
  →  &nbsp;이 데이터에서는 트리 모델이 가장 적합한데, 그렇다면 스케일링이 크게 중요하지 않으므로 스케일링 과정은 생략하였다.
